...
void menu(Products *products_list){      // subrotina para printar as bordas do menu
  ...
  for(cont=3; cont<77; cont++){
    ...
    gotoxy(cont, 9); printf("%c", 205);   //printar as linhas horizontais
  }

  for(cont=2; cont<9; cont++){
      gotoxy(77, cont); printf("%c", 186);   // printar as colunas
  ...
  while(1){
    if(kbhit()){
      switch(getch()){
        ...
        case 13:
          return cy;    // retorna a coordenada do cursor
      }
    }
...
void show_itens (Products *products_list, int *i) {
  ...
  if (empty_list(products_list)) {
    ...
    gotoxy(32, 11);puts("List is empty.");
    return 0;
  }
  ...
  *i=11;
  ...
  while (tmp != NULL){
    gotoxy(77, *i-1);printf("%c", 186);
    gotoxy(2, *i-1);printf("%c", 186);
    gotoxy(77, *i+1);printf("%c", 186);
    gotoxy(2, *i+1);printf("%c", 186);
    gotoxy(77, *i+2);printf("%c", 186);
    gotoxy(2, *i+2);printf("%c", 186);
    gotoxy(77, *i);printf("%c", 186);
    gotoxy(2, *i);printf("%c", 186);
    gotoxy(32, *i);printf("Code: %d", tmp->code);
    gotoxy(32, *i+1);printf("Price: %.2f", tmp->price);
  ...
  gotoxy(2, *i); printf("%c", 200);
  gotoxy(77, *i); printf("%c", 188);
    ...
    switch (cursor(3, 3, 7, 1)) {
      case 5:
        ...
        if (check_list(x, products_list)){
          gotoxy(55, 5);puts("Product not found");
      ...
      case 3:
        ...
        if (insert_item(&code, &price, products_list) != NULL){
          ...
          gotoxy(50, 5);puts("The product already exist");
          gotoxy(48, 8);puts("Press any key to continue...");
          getch();
        }
      break;
      ...
      case 6:
        if(empty_list(products_list)){
          gotoxy(26, 13);puts("Press any key to continue...");
          getch();
        }else{
          gotoxy(55, 12);puts("Press enter to exit");
          cursor(11, 11, i-3, 3);
        }
      break;
